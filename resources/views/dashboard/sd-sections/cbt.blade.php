...
<div class="card" style="margin-bottom:14px">
    <div class="card-header">
        ...
        <div style="display:flex;gap:8px;align-items:center">
                    <span class="badge {{ $exam->tipe === 'uts' ? 'orange' : ($exam->tipe === 'uas' ? 'purple' : 'blue') }} light" style="font-size:10px">{{ strtoupper($exam->tipe) }}</span>
                    <span class="badge blue">{{ $exam->mapel->kode ?? '-' }}</span>
                    <span class="badge blue">{{ $exam->jumlah_soal }} soal</span>
    ...
    <div style="margin-top:10px;display:flex;align-items:center;justify-content:space-between">
        <span style="font-size:13px;color:var(--gray-400)">
            ...
            <i class="fas fa-check-circle" style="color:var(--blue)"></i> Sudah dikerjakan
            @else
            <i class="fas fa-clock" style="color:var(--orange)"></i> Belum dikerjakan
            @endif
        </span>
        @if($exam->sudah_dikerjakan)
            <a href="{{ route('siswa.cbt.hasil', $exam->id) }}" class="btn-small blue" style="text-decoration:none; color:white;"><i class="fas fa-eye"></i> Lihat Hasil</a>
        @else
            <a href="{{ route('siswa.cbt.kerjakan', $exam->id) }}" class="btn-small blue" style="text-decoration:none; color:white;"><i class="fas fa-pencil-alt"></i> Kerjakan</a>
        @endif
    </div>
</div>
@empty
