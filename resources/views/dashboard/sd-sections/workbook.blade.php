...
<div x-data="{ filterTipeWB: 'semua' }">
    ...
    <div class="card-header">
        <h3><i class="fas fa-book" style="color:var(--blue)"></i> {{ $wb->judul }}</h3>
        <div style="display:flex;gap:8px;align-items:center">
            <span class="badge blue">{{ $wb->mapel->kode ?? $wb->mapel->nama_mapel ?? '-' }}</span>
            <span class="badge blue">{{ $wb->soals_count }} soal</span>
        </div>
    </div>
    ...
    <div style="margin-top:10px;display:flex;align-items:center;justify-content:space-between">
        ...
        @if($wb->dijawab > 0 && $wb->dijawab >= $wb->soals_count)
            <a href="{{ route('siswa.workbook.hasil', $wb->id) }}" class="btn-small blue" style="text-decoration:none; color:white;"><i class="fas fa-eye"></i> Lihat Hasil</a>
        @else
            <a href="{{ route('siswa.workbook.kerjakan', $wb->id) }}" class="btn-small blue" style="text-decoration:none; color:white;"><i class="fas fa-pencil-alt"></i> Kerjakan</a>
        @endif
    </div>
</div>
@empty
