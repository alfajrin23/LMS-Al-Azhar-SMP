...
<div x-data="{ showBandingModal: false, activeNilaiId: null, alasan: '' }">
    ...
    <div class="grid-2">
        <div class="card">
            <div class="card-header">
                <h3><i class="fas fa-file-invoice" style="color:var(--blue)"></i> Rapor Semester Genap</h3><label
                    @click="tab='rapor'"
                    style="font-size:12px;color:var(--blue);font-weight:600;cursor:pointer;text-decoration:none">Cetak</label>
            </div>
            @foreach ($nilai as $n)
                ...
                <div class="rapor-item" style="flex-wrap: wrap; gap: 8px;">
                    <div style="display:flex; flex-direction:column;">
                        ...
                        @if ($n->nilai_bahasa)
                            <span style="font-size:11px; color:#4f46e5; font-weight:600">Komponen B. Inggris:
                                {{ $n->nilai_bahasa }}</span>
                        @endif
                    </div>
                            ...
                            @if ($n->banding)
                                @if ($n->banding->status === 'pending')
                                    <span class="status-badge"
                                        style="background-color: #fffbeb; color: #d97706; border: 1px solid #fef3c7;"
                                        title="Alasan: {{ $n->banding->alasan_siswa }}">⏳ Banding</span>
                                @elseif($n->banding->status === 'disetujui')
                                    <span class="status-badge"
                                        style="background-color: #f0fdf4; color: #16a34a; border: 1px solid #dcfce7;"
                                        title="Catatan Guru: {{ $n->banding->catatan_guru ?? 'Disetujui' }}">✅
                                        Disetujui</span>
                                @elseif($n->banding->status === 'ditolak')
                                    <span class="status-badge"
                                        style="background-color: #fef2f2; color: #dc2626; border: 1px solid #fee2e2;"
                                        title="Catatan Guru: {{ $n->banding->catatan_guru ?? 'Ditolak' }}">❌
                                        Ditolak</span>
                                @endif
                            @else
                                <button type="button"
                                    @click="activeNilaiId = {{ $n->id }}; alasan = ''; showBandingModal = true"
                                    style="background:#4f46e5; color:white; border:none; padding:4px 10px; border-radius:12px; font-size:11px; font-weight:700; cursor:pointer; display:inline-flex; align-items:center; gap:4px">
                                    ⚖️ Banding
                                </button>
                            @endif
                        @endif
        ...
        <div class="card" x-data="{
            ...
            <div class="card-header"
                ...
                <h3 style="margin: 0;"><i class="fas fa-chart-bar" style="color:var(--teal)"></i> Statistik Per Bab</h3>
    ...
    <div x-show="showBandingModal" class="modal-backdrop"
        ...
        x-cloak>
        <div @click.away="showBandingModal = false"
            style="background:white; border-radius:12px; width:100%; max-width:500px; padding:24px; box-shadow:0 10px 25px rgba(0,0,0,0.1); position:relative">
            <h3 style="margin-top:0; color:#1e293b; display:flex; align-items:center; gap:8px"><i
            ...
            <form action="{{ route('siswa.banding.store') }}" method="POST">
                ...
                <textarea name="alasan_siswa" x-model="alasan"
                    placeholder="Contoh: Saya sudah mengerjakan dengan rumus yang benar, mohon review kembali porsi nilai bahasa..."
                ...
                <div style="display:flex; justify-content:flex-end; gap:12px; margin-top:20px">
                    ...
                    <button type="submit" :disabled="alasan.length < 5"
                        style="background:#4f46e5; border:none; padding:10px 16px; border-radius:8px; cursor:pointer; font-weight:600; color:white; display:inline-flex; align-items:center; gap:6px; transition:opacity 0.2s"
                        :style="alasan.length < 5 ? 'opacity:0.5; cursor:not-allowed;' : ''">Kirim Banding</button>
                </div>
            </form>
        </div>
    </div>
